...
double AdamsBashforthCorrector(double (*f)(double, double), double a, double b, double y0) {
    double h = (b - a) / N;
    ...
    for (int i = 3; i < N; i++) {
        ...
        double y_predict = y[i] + h / 24.0 * (
            55 * f_val[i] - 59 * f_val[i - 1] + 37 * f_val[i - 2] - 9 * f_val[i - 3]);

        // Corrector: 4-step Adams-Moulton
        double f_predict = f(x[i + 1], y_predict);
        y[i + 1] = y[i] + h / 24.0 * (
            9 * f_predict + 19 * f_val[i] - 5 * f_val[i - 1] + f_val[i - 2]);
...
int main() {
    ...
    double result = AdamsBashforthCorrector(f[choice - 1], x0[choice - 1], x_target[choice - 1], y0[choice - 1]);
